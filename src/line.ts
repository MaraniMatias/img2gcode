export default class Line {
  private _sign: { x: string; y: string} = {x:'-',y:'-'}; //en file esta como Y-
  /*
   * @param  {x?:number; y?:number; z?:number;} axes
   * @param  {string} comment?
   */
  constructor(axes: ImgToGCode.Axes, invest: { x: boolean; y: boolean }, comment?: string) {
    this._axes = axes;
  ...
  public code(percentage: number): string {
    let x = (this._axes.x!==undefined && ` X${this._sign.x}${this._axes.x.toFixed(4)}`) || '';
    let y = (this._axes.y!==undefined && ` Y${this._sign.y}${this._axes.y.toFixed(4)}`) || '';
